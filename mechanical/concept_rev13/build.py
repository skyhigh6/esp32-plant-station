"""Plant Station R13. Millimetres; x right, y rear, z up. Run with local cad-runtime."""
...
def verify():
    report={'revision':'R13','units':'mm','cadquery':cq.__version__,'parts':{},'pairwise':[],'critical':{}}
    ...
    report['critical']={'integrated_body':True,'cable_route':{'provisional_cable_OD_mm':4,'tube_bore_mm':10,'pump_wire_bore_mm':8,'common_intake_width_height_mm':[23.5,34],'tube_exit_z_mm':128,'sensor_entry_centre_yz_mm':[65,64],'sensor_bore_mm':8,'crossover_bottom_z_mm':77,'sump_rim_z_mm':60,'crossing_min_z_mm':77,'water_level':'UNKNOWN - no approved maximum fill','connector_and_bend_radius':'UNKNOWN - route unterminated cable via open top; two turns require physical trial'},'edge_treatment_mm':{'cavity_vertical':2,'cavity_floor':1,'rim_base':0.5,'front_edge':0.3,'support_profile':0.6,'standoff_root':0.6,'standoff_tip_chamfer':0.3},'upper_support_top_z_mm':158,'LCD_aperture_mm':LCD_APERTURE_MM,'LCD_centres_mm':LCD_CENTRES_MM,'PCB_reference_centres_mm':PCB_CENTRES_MM,'PCB_installed_centres_xz_mm':[57,60],'plant_cavity_mm':[90,60,50],'drains':2,'drain_diameter_mm':6,'floor_mm':4,'dovetail_clearance_mm':0.4,'through_probes':'PASS','floor_probe':'PASS','overlap_control_mm3':500}
    ...
    # Provisional tube plus two individual wire envelopes in separate bores.
    # Entry opening permits a tube approach from the sump; bends unverified.
    route_probes=[cyl(123,92,40,4,105),cyl(123,70,30,4,22,(0,1,0)),
                  cyl(108.5,92,30,1.25,51),cyl(111.5,92,30,1.25,51),
                  cyl(85,91,79.5,1.25,25,(1,0,0)),cyl(85,91,82.5,1.25,25,(1,0,0)),
                  cyl(85,65,64,2,20,(1,0,0))]
    for probe_index,probe in enumerate(route_probes):
        for name,sh in parts.items():
            assert sh.intersect(probe).Volume()<1e-5,('cable route blocked',name,probe_index,sh.intersect(probe).Volume())
    ...
    tie_probe=box(45.1,29,7.1,5.8,32,2.3)
    assert combined.intersect(tie_probe).Volume()<1e-5, 'Cable tie tunnel blocked'
    tie_roof=box(45,32,9.6,6,26,2.3)
    assert abs(combined.intersect(tie_roof).Volume()-tie_roof.Volume())<1e-5, 'Tie tunnel roof missing'
    holder_space=box(12.1,34.1,12.01,71.8,21.8,8.9)
    assert combined.intersect(holder_space).Volume()<1e-5, 'Battery holder clearance reduced'
    report['battery_tie']={'integrated':True,'slot_width_height_mm':[6,2.5],'slot_x_range_mm':[45,51],'slot_y_range_mm':[29,61],'slot_z_range_mm':[7,9.5],'provisional_tie_width_thickness_mm':[4.8,1.5],'through_path_roof_holder_probes':'PASS'}
    support=box(101,85,4,3,2,115)
    assert abs(combined.intersect(support).Volume()-support.Volume())<1e-5, 'Riser support discontinuity'
    assert combined.intersect(box(135,86,65,40,8,25)).Volume()<1e-5, 'Old floating trough remains'
    lift_checks=[]
    ...
    report['cable_checks']={'bridge_continuity_probe':'PASS','tube_and_two_wire_and_sensor_probes':'PASS','full_height_sump_wall_probe':'PASS','rim_to_crossover_bottom_mm':17,'routing':'supported common service area; separated tube and pump-wire bores; independent sensor entry at sump rim'}
    for x in (18,78):
        probe=cyl(x,45,9.6,1.6,2.3)
        assert abs(combined.intersect(probe).Volume()-probe.Volume())<1e-5
    for x in (14.1,81.7):
        probe=box(x,5,6.05,.2,7,.3)
        assert abs(combined.intersect(probe).Volume()-probe.Volume())<1e-5
    assert combined.intersect(box(96.1,84,132,8,14,8)).Volume()<1e-5
    assert combined.intersect(cyl(93,65,128,3,2,(1,0,0))).Volume()>56
    report['annotation_checks']={'clip_and_shelf_removed':True,'battery_holes_filled':True,'lower_boss_concave_root_radius_mm':2,'riser_vertical_radius_mm':7,'riser_top_radius_mm':3,'sensor_lower_edge_z_mm':60,'material_and_void_probes':'PASS'}
