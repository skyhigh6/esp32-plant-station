...
report={'revision':'R10','units':'mm','files':{}}
...
assert len(report['files'])==21
assy=cq.importers.importStep(str(OUT/'assembly.step')).val()
assert assy.isValid() and len(assy.Solids())==6
report['assembly_solids']=6
report['result']='PASS'
(OUT/'step_check.json').write_text(json.dumps(report,indent=2))
print('PASS: reopened 21 STEP single solids and six-part assembly; STL bounds/volumes agree',flush=True)
