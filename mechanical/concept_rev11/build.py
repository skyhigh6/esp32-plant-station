"""Plant Station R11. Millimetres; x right, y rear, z up. Run with local cad-runtime."""
...
# R11: one structural base; retain both water-containing walls below the rim.
...
riser=rounded(94,84,4,25,13,86,1)
combined=fuse(tower,sump,bridge,riser)
# Open vertical top assists threading; no separate cantilever or loose cover.
combined=cut(combined,cyl(113,90.5,20,4,72),
             cyl(113,90.5,24,4,9,(1,0,0)),
             cyl(89,90.5,81,4,24,(1,0,0)))
# Local planter relief clears the column and existing hose-clip shelf on lifting.
plant=cut(plant,box(101,83,56,19,18,70))
...
def verify():
    report={'revision':'R11','units':'mm','cadquery':cq.__version__,'parts':{},'pairwise':[],'critical':{}}
    ...
    route_probes=[cyl(113,90.5,24,2,57),cq.Solid.makeSphere(2,V(113,90.5,24)),cq.Solid.makeSphere(2,V(113,90.5,81)),cyl(113,90.5,24,2,20,(1,0,0)),cyl(85,90.5,81,2,28,(1,0,0))]
    ...
    support=box(108,85,4,2,2,70)
    assert abs(combined.intersect(support).Volume()-support.Volume())<1e-5, 'Riser support discontinuity'
    assert combined.intersect(box(125,86,65,50,8,25)).Volume()<1e-5, 'Old floating trough remains'
    lift_checks=[]
    ...
    report['cable_checks']={'bridge_continuity_probe':'PASS','connected_4mm_route_probes':'PASS','full_height_sump_wall_probe':'PASS','rim_to_crossover_bottom_mm':17,'routing':'floor and wall integrated column, no spanning trough'}
