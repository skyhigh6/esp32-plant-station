"""Plant Station R9. Millimetres; x right, y rear, z up. Run with local cad-runtime."""
...
def verify():
    report={'revision':'R9','units':'mm','cadquery':cq.__version__,'parts':{},'pairwise':[],'critical':{}}
    ...
    report['critical']={'edge_treatment_mm':{'cavity_vertical':2,'cavity_floor':1,'rim_base':0.5,'front_edge':0.3,'support_profile':0.6,'standoff_root':0.6,'standoff_tip_chamfer':0.3},'upper_support_top_z_mm':158,'LCD_aperture_mm':LCD_APERTURE_MM,'LCD_centres_mm':LCD_CENTRES_MM,'PCB_reference_centres_mm':PCB_CENTRES_MM,'PCB_installed_centres_xz_mm':[57,60],'plant_cavity_mm':[90,60,50],'drains':2,'drain_diameter_mm':6,'floor_mm':4,'dovetail_clearance_mm':0.4,'through_probes':'PASS','floor_probe':'PASS','overlap_control_mm3':500}
        ...
        for name,sh in shapes.items():
            if mode=='exploded':
                delta={'fascia_charcoal':(0,-38,0),'top_cover_sage':(0,0,40),'planter_sage':(25,0,55),
                       'sump_charcoal':(25,0,0),'battery_tray':(0,-25,20),'hose_clip':(0,0,15)}.get(name,(0,0,0))
