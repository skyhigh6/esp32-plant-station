...
report={'units':'mm','revision':'R12','files':{},'sections':{}}
meshes={}
...
report['sections']['supported_riser_bore']=bore_section(meshes['tower_sump_body'],[0,0,70],[0,0,1],[[110,92],[123,92]],[0,1],[8,10])
loops=meshes['tower_sump_body'].section(plane_origin=[0,45,0],plane_normal=[0,1,0]).discrete
slots=[q[:,[0,2]] for q in loops if np.allclose(q[:,[0,2]].min(axis=0),[45,7],atol=.025) and np.allclose(q[:,[0,2]].max(axis=0),[51,9.5],atol=.025)]
assert len(slots)==1,'Battery tie slot section mismatch'
report['sections']['battery_tie_slot']={'width_mm':6,'height_mm':2.5,'plane_y_mm':45,'result':'PASS'}
report['sections']['sensor_entry']=bore_section(meshes['tower_sump_body'],[93,0,0],[1,0,0],[[65,128]],[1,2],[8])
...
print('PASS: 20 exported meshes; rotated mount pattern, USB opening, piezo/LED seats and knob sections',flush=True)
